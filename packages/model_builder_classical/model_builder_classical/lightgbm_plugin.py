...
class LightGBMPlugin:
    ...
    def detect(self, profile: DataProfile) -> float:
        if "tabular" not in profile.data_types:
            return 0.0
        return 0.88 if profile.row_count > 5000 else 0.80
    ...
    def train(self, data: DataBundle, config: dict, callbacks: TrainCallbacks) -> Artifact:
        ...
        callbacks.on_log(f"LightGBM: {len(df)} rows, task={task.value}")
        callbacks.on_progress(0, 1, {})

        n = config.get("n_estimators", 100)
        lr = config.get("learning_rate", 0.1)
        params = {"n_estimators": n, "learning_rate": lr, "random_state": 42, "verbose": -1}

        model = lgb.LGBMRegressor(**params) if task == TaskType.REGRESSION else lgb.LGBMClassifier(**params)
        model.fit(X, y)
        callbacks.on_progress(1, 1, {})

        model_dir = Path(data.data_path).parent / "lgbm_model"
    ...
    def update(self, artifact: Artifact, data: DataBundle, config: dict, callbacks: TrainCallbacks) -> Artifact:
        """Incremental LightGBM training from existing model checkpoint."""
        with open(artifact.model_path, "rb") as f:
            saved = pickle.load(f)
        ...
        new_model.fit(X, y, init_model=old_model)
        callbacks.on_log(f"LightGBM incremental: +{n} rounds from existing model")
        callbacks.on_progress(1, 1, {})

        model_path = Path(artifact.model_path).parent / "model.pkl"
        with open(model_path, "wb") as f:
            pickle.dump({"model": new_model, "X_cols": list(X.columns), "task": task.value}, f)
        return Artifact(model_path=str(model_path), plugin_name="lightgbm",
